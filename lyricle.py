...
class Lyricle:
    ...
    def generate_lyricles_from_playlist(playlist, limit=0):
        ...
        for i, track in enumerate(tracks[:limit]):
            print(f"Processing {i + 1} of {limit + 1} songs")
            cleaned = SpotifyAPIClient.clean_tags(track[0])
            try:
                song_data = Lyricle.generate_lyricle(cleaned, track[1].split(", ")[0])
                song_data.artist = track[1]
            except:
        ...
        # * Lint Covers
        result = LyricLinter.remove_cover_songs(result)
        # print(result)
        return result
    ...
    def generate_lyricle(song_name, artist_name):
        ...
        try:
            lyrics = LyricsGeniusAPIClient().get_lyricle_list(song_name, artist_name)
        except NameError:
