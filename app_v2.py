...
from lyricle_v2 import Lyricle
...
def main(argv):
    ...
    if FLAGS.path is not None:
        ...
        if FLAGS.output is not None:
            ...
            return
    try:
        track = SpotifyAPIClient.search_track(FLAGS.title, FLAGS.artist)
        ...
        raise SongDoesntExistError(
            f"Song {FLAGS.title} doesn't exist for the artist {FLAGS.artist} please correct your input."
        )
    except:
